...
########## SET OUTPUT DIR NAME
output_dir = f'/home/m10921061/scratch/output/OceanSciencesPoster_d100'
...
bbox_ross = (172.77124,-81,174.30194,-80.7238)
...
for i, const in enumerate(constituents): # Loop over amplitudes
    ...
    cbar.set_label(f"{const} Data Amplitude [m]", fontsize=28, color="black")
...
for i, const in enumerate(constituents): # Loop over amplitudes
    ...
    amps_pyTMD = np.array([a[i] for a in tide_analyses['amp_pyTMD'][:n]])
    amp_diff = amps - amps_pyTMD
    col = ax.scatter(xs, ys, s=0.1, zorder=10, c=amps_pyTMD, cmap='viridis',vmin=hmin,vmax=hmax)
    cbar = fig.colorbar(col)
    ...
    cbar.ax.tick_params(labelsize=25)
    cbar.set_label(f"{const} Modeled Amplitude [m]", fontsize=28, color="black")
    fig.savefig(f"{output_dir}/{const}_modeled_amp.png", bbox_inches="tight", dpi=300, transparent=True)
